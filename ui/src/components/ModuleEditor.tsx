...
const ModuleEditor = ({ module, path }: ModuleEditorProps) => {
  ...
  function renderField(key: string | number, sch: any, rootSchema: any, pathInData: Path) {
    ...
    // Use Controller for controlled inputs connected to RHF
    ...
    if (type === "array" && resolvedSch.items) {
      ...
      // For array without x-block-type, fallback to JSON textarea editing with Controller
      return (
        <div key={String(key)}>
    ...
    // Default string input using Controller
    return (
      <div key={String(key)}>
