...
type DiffType = "event" | "context" | "result";
...
export default function TraceDetailPanel({ entry, className }: TraceDetailPanelProps) {
    ...
    useEffect(() => {
        if (entry?.category === "result") {
            setDiffType("result");
        } else if (diffType === "result") {
            setDiffType("event");
        }
    }, [entry?.id]);
    ...
    const { beforeContent, afterContent, hasEventData, hasContextData, hasResultData } = useMemo(() => {
        if (!entry) {
            return { beforeContent: "{}", afterContent: "{}", hasEventData: false, hasContextData: false, hasResultData: false };
        }
        ...
        const isResult = entry.category === "result";
        const resultStr = isResult ? JSON.stringify((entry as any).result, null, 2) : "{}";
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        ...
                        <div className="flex text-xs text-gray-500 mb-1 shrink-0">
                            <div className="flex-1 flex items-center gap-1">
                                ...
                                <span>{diffType === "result" ? "Result Value" : "Before → After"}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 min-h-0 border rounded overflow-hidden">
                            <DiffEditor
                                ...
                                options={{
                                    ...
                                    renderSideBySide: diffType !== "result",
                ...
                <DialogContent className="max-w-[95vw] w-[95vw] sm:max-w-none max-h-[95vh] h-[95vh] flex flex-col p-0 gap-0">
                    <DialogHeader className="shrink-0 px-6 pt-6 pb-4 border-b">
                        <DialogTitle className="flex items-center justify-between">
                            <span className="text-lg">{entry.alias || entry.type || "Unknown"} - {diffType === "result" ? "Result Value" : "Diff View"}</span>
                            <div className="flex bg-gray-200 rounded p-0.5 mr-8">
                                {hasResultData && (
                                    <button
                                        onClick={() => setDiffType("result")}
                                        className={`px-4 py-1.5 text-sm font-medium rounded transition-colors ${diffType === "result"
                                            ? "bg-white text-gray-900 shadow-sm"
                                            : "text-gray-500 hover:text-gray-700"
                                            }`}
                                    >
