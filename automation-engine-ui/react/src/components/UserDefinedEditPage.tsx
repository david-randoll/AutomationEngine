...
interface UserDefinedEditPageProps {
    blockType: BlockType;
    name: string;
}
...
const UserDefinedEditPage = ({ blockType, name }: UserDefinedEditPageProps) => {
    const navigate = useNavigate();
    const { setValue, getValues } = useFormContext();
    const rootPath: Path = ["edit"];
    ...
    useEffect(() => {
        const loadData = async () => {
            setIsLoading(true);
            try {
                // Fetch the schema and existing definition in parallel
                const [schemaData, definitionData] = await Promise.all([
                    userDefinedApi.getSchema(blockType),
                    userDefinedApi.get(blockType, name),
                ]);

                setSchema(schemaData);
                // Set the form data with the existing definition
                setValue(pathKey, definitionData);
            } catch (err) {
        ...
        loadData();
    }, [blockType, name, setValue, pathKey]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error(`Failed to update ${blockTypeLabels[blockType].toLowerCase()}`);
        } finally {
            ...
            <div className="p-4 sm:p-6 bg-gray-50 min-h-screen">
                <div className="max-w-7xl mx-auto">
                    <div className="text-center py-12">
                        ...
                        <Button variant="outline" className="mt-4" onClick={() => navigate({ to: "/user-defined/$blockType", params: { blockType } })}>
            ...
            <div className="max-w-7xl mx-auto">
                <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-2 sm:gap-0">
                    <div className="flex items-center gap-4">
                        <Button variant="ghost" size="sm" onClick={() => navigate({ to: "/user-defined/$blockType", params: { blockType } })}>
                            <ArrowLeftIcon className="size-4" />
                        </Button>
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold">
                                Edit {blockTypeLabels[blockType]}: {name}
                            </h1>
                            <p className="text-sm text-gray-500">
                                Modify the user-defined {blockTypeLabels[blockType].toLowerCase()}
                            </p>
                        </div>
                    </div>
...
export default UserDefinedEditPage;
