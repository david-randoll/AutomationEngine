...
class VariableTemplatingInterceptorTest {

    @Mock
    private JsonNodeVariableProcessor processor;

    private ObjectMapper objectMapper;
    private VariableTemplatingInterceptor interceptor;

    @BeforeEach
    void setUp() {
        objectMapper = new ObjectMapper();
        interceptor = new VariableTemplatingInterceptor(processor, objectMapper);
    }
    ...
    void testIntercept() {
        ...
        data.put("key", "value");
        VariableContext variableContext = new VariableContext(null, null, null, data);

        IVariableChain chain = mock(IVariableChain.class);
        ...
        verify(chain).resolve(eq(eventContext), any(VariableContext.class));
    }
}
