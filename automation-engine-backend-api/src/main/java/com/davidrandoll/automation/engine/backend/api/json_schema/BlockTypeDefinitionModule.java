...
public class BlockTypeDefinitionModule implements Module {
    ...
    private String getBlockType(Class<?> type) {
        if (TriggerDefinition.class.isAssignableFrom(type)) {
            return "trigger";
        }
        if (ConditionDefinition.class.isAssignableFrom(type)) {
            return "condition";
        }
        if (ActionDefinition.class.isAssignableFrom(type)) {
            return "action";
        }
        if (VariableDefinition.class.isAssignableFrom(type)) {
            return "variable";
        }
        return null;
    }
